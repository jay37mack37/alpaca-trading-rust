    ...
    try {
        ...
        ordersBody.innerHTML = data.slice(0, 20).map(order => {
            ...
            <tr data-order-id="${order.id}">
                <td>
                    ${isOpen ? `<input type="checkbox" class="order-checkbox" data-order-id="${order.id}">` : ''}
                </td>
...
export async function cancelOrder(orderId) {
    ...
    try {
        ...
        devLog('ORDER', 'Order cancelled successfully', orderId);
        fetchOrders();
    } catch (err) {
...
export async function cancelAllOrdersInternal() {
    try {
        ...
        const data = response._body;
        if (!response.ok) throw new Error(data.error || data.message || 'Failed to cancel orders');
...
export async function cancelSelectedOrders() {
    ...
    if (!confirm(`Are you sure you want to cancel ${orderIds.length} selected orders?`)) return;

    for (const orderId of orderIds) {
        try {
            await fetchWithLogging(`${API_BASE}/api/orders/${orderId}`, {
                method: 'DELETE',
                headers: getAuthHeaders()
            });
        } catch (err) {
