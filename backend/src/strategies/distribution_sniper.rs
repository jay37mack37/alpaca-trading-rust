use crate::models::{
    Candle, Quote, StrategyRecord, StrategySignal, TradeLeg, TradeSide,
};
    ...
    async fn evaluate(
        ...
        crate::agents::broadcast_audit_log(
            ...
            crate::logger::SystemEvent::now(
                ...
                0.5,
                format!(
                    "DIVIDEND SCAN: Checking yield capture feasibility for {}.",
                    quote.symbol
                ),
            ),
        );
        ...
        let dividend_info =
            match CorporateActionsService::fetch_dividend_info(&state.http, &quote.symbol).await {
                Ok(Some(info)) => info,
                _ => {
                    crate::agents::broadcast_audit_log(
                        state,
                        crate::logger::SystemEvent::now(
                            ...
                            0.5,
                            format!(
                                "NO DIVIDEND: {} has no upcoming payouts in database.",
                                quote.symbol
                            ),
                        ),
                    );
        ...
        if time_to_ex > Duration::hours(0) && time_to_ex < Duration::hours(24) {
            ...
            if let Some(put) = best_put {
                ...
                if total_hedge_cost < total_dividend {
                    let mut signal = buy(
                        format!(
                            "Dividend Capture Target: ${:.2} (Hedge Cost: ${:.2})",
                            total_dividend, total_hedge_cost
                        ),
                        0.15, // 15% allocation
                        Some(quote.price),
                    );
                    ...
                    return hold(format!(
                        "Hedge too expensive: ${:.2} vs ${:.2} div",
                        total_hedge_cost, total_dividend
                    ));
                }
            }
        }
        ...
        hold(format!(
            "Monitoring ex-date: {}",
            dividend_info.ex_dividend_date.format("%Y-%m-%d")
        ))
    }
}
