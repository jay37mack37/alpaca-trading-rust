...
pub fn evaluate_parity_sniper(
    ...
    if let Some(pos) = position {
        ...
        let matching_call = options
            .iter()
            .find(|o| o.contract_symbol == pos.instrument_symbol);
        ...
        if let (Some(call), Some(put)) = (matching_call, matching_put) {
            ...
            let exp_date = chrono::NaiveDate::parse_from_str(
                pos.expiration.as_deref().unwrap_or(""),
                "%Y-%m-%d",
            )
            .unwrap_or(today);
            ...
            let current_edge = if strike > 0.0 {
                current_gap / strike
            } else {
                0.0
            };
            ...
            if current_edge < 0.005 {
                return StrategySignal {
                    ...
                    reason: format!(
                        "PROACTIVE EXIT: Parity gap has converged (Edge: {:.2}%)",
                        current_edge * 100.0
                    ),
            ...
            if let Some(put) = matching_put {
                ...
                if call_price > 0.0 && put_price > 0.0 {
                    ...
                    if gap > best_gap {
                        ...
                        best_context = format!(
                            "S:{:.2} C:{:.2} P:{:.2} K:{:.0}",
                            spot_price, call_price, put_price, contract.strike
                        );
                    }
                }
            }
    ...
    let edge_pct = if best_strike > 0.0 {
        best_gap / best_strike
    } else {
        0.0
    };

    // SYSTEM LOGGING: Decision Support System
    let event_type = if edge_pct >= 0.01 {
    ...
    let narrative = if edge_pct >= 0.01 {
        format!(
            "CRITICAL: {:.2}% Parity Gap Found. Verified arbitrage path at strike {:.0}.",
            edge_pct * 100.0,
            best_strike
        )
    } else {
    ...
    if edge_pct > 0.01 {
        return StrategySignal {
            ...
            reason: format!(
                "ARBITRAGE: Parity Gap of {:.1}% detected at strike {:.0}",
                edge_pct * 100.0,
                best_strike
            ),
            source: Some("PARITY_SNIPER".to_string()),
