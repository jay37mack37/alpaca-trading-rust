...
impl Database {
    pub fn open(path: &Path, default_watchlist: &[String], master_key: &str) -> AppResult<Self> {
        ...
        // PRAGMA statements return rows in SQLite, so use pragma_update instead of execute.
    ...
    fn seed_default_strategies(&self) -> AppResult<()> {
        ...
        {
            let default_ids: Vec<&str> = defaults.iter().map(|(id, _, _, _)| *id).collect();
            let placeholders = default_ids
                .iter()
                .map(|_| "?")
                .collect::<Vec<_>>()
                .join(",");
            let query = format!(
                "SELECT id, kind FROM strategies WHERE id IN ({})",
                placeholders
            );
            let mut stmt = self.conn.prepare(&query)?;
            let params = rusqlite::params_from_iter(default_ids);
            let mut rows = stmt.query(params)?;
            while let Some(row) = rows.next()? {
                let id: String = row.get(0)?;
                let kind: String = row.get(1)?;
                existing_strategies.insert(id, kind);
            }
        }
        ...
        for (id, name, kind, symbols) in defaults {
            ...
            if let Some(old_kind) = existing_strategies.get(id) {
                println!(
    ...
    pub fn store_broker_sync(
        ...
        for strategy_id in strategies_to_sync {
            ...
            let positions_to_reconcile: Vec<PositionRecord> = stmt
...
mod bench_tests {
    ...
    #[test]
    fn bench_seed_default_strategies() {
        let db = Database::open(Path::new(":memory:"), &[], "test_key").unwrap();

        let start = Instant::now();
        for _ in 0..100 {
            db.seed_default_strategies().unwrap();
        }
