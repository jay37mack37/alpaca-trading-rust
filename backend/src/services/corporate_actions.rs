...
impl CorporateActionsService {
    ...
    pub async fn fetch_dividend_info(client: &Client, symbol: &str) -> AppResult<Option<DividendInfo>> {
        let url = format!("https://query2.finance.yahoo.com/v10/finance/quoteSummary/{}?modules=summaryDetail", symbol);
    ...
    pub async fn fetch_alpaca_dividends(
        ...
        let url = format!("{}/v2/corporate-actions?symbols={}", credential.environment.base_trading_url(), symbol);
        let response = client.get(url)
            ...
            for action in actions {
                if action["ca_type"].as_str() == Some("dividend") {
                    if let Some(ex_date_str) = action["ex_date"].as_str() {
                        ...
                        if let Ok(ex_date) = DateTime::parse_from_rfc3339(&format!("{}T00:00:00Z", ex_date_str)) {
