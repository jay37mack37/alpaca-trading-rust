...
use chrono::Utc;
use db::{Database, LocalTradeInput};
use error::{AppError, AppResult};
use models::{
    AssetClassTarget, BrokerSyncState, CollectResponse, CreateCredentialRequest,
    CreateStrategyRequest, DashboardResponse, DataProvider, ExecutionMode,
    OptionContractSnapshot, OptionEntryStyle, OptionStructurePreset, RealtimeEvent,
    SignalAction, StrategyDetailResponse, StrategyRecord, StrategySignal, TradeLeg, TradeSide,
    UpdateStrategyRequest, WatchlistAddRequest,
};
use providers::{
...
struct AppState {
    ...
    config: AppConfig,
    streams: StreamHub,
    agent_tasks: Arc<Mutex<std::collections::HashMap<String, tokio::task::JoinHandle<()>>>>,
}
...
struct PreparedTrade {
    local: LocalTradeInput,
    broker_order: Option<AlpacaOrderRequest>,
}

#[derive(Debug, Clone)]
struct ResolvedOptionContract {
...
async fn main() -> anyhow::Result<()> {
    ...
    // The master key is consumed exactly once to derive the Argon2id credential
    // cipher, then dropped so it never sits in AppState.
    ...
    // Drop the master key eagerly so it doesn't linger in process memory.
    drop(master_key);
    ...
    let app = Router::new()
        ...
        .route("/api/stream", get(realtime_stream))
        .route("/api/market/quote/:symbol", get(market_quote))
        .route("/api/market/candles/:symbol", get(market_candles))
        .route("/api/options/:symbol", get(options_chain))
        .route(
            "/api/watchlists",
        ...
        .route(
            "/api/watchlists/:id",
            axum::routing::put(update_watchlist).delete(delete_watchlist),
        )
        .route(
            "/api/credentials",
        ...
        .route(
            ...
            get(strategy_detail).patch(update_strategy),
        )
        .route(
            "/api/strategies/:strategy_id/alpaca-sync",
            post(sync_strategy_broker),
        )
        .route("/api/strategies/:strategy_id/run", post(run_strategy))
        .route("/api/watchlist", post(add_watchlist_symbol))
        .route("/api/watchlist/:symbol", delete(remove_watchlist_symbol))
        .route("/api/collect", post(collect_now))
        .route("/api/robinhood/ingest", post(ingest_robinhood_data))
...
async fn ingest_robinhood_data(
    State(_state): State<AppState>,
    Json(payload): Json<RobinhoodIngestPayload>,
) -> Json<serde_json::Value> {
    // For now, we simply log the ingested data.
    // In the future, this can be parsed as a realtime data source,
    // converted into models::Quote or models::OptionContractSnapshot,
    // and broadcasted via the stream or written to the DB.
    info!(
    ...
    // Example: if we wanted to broadcast this to connected frontend clients,
    // we could send a RealtimeEvent here using state.broadcast.
...
) -> AppResult<Json<Vec<models::Candle>>> {
    let provider = query.provider.unwrap_or(DataProvider::Yahoo);
    let symbol = normalize_symbol(&symbol);
    let credential = match provider {
        DataProvider::Yahoo => None,
...
async fn options_chain(
    ...
    Path(symbol): Path<String>,
    Query(query): Query<ProviderQuery>,
) -> AppResult<Json<Vec<models::OptionContractSnapshot>>> {
    let symbol = normalize_symbol(&symbol);
    let provider = query.provider.unwrap_or(DataProvider::Yahoo);
    let credential = match provider {
        DataProvider::Yahoo => None,
    ...
    Ok(Json(top_option_contracts(fetched.contracts)))
}
...
async fn list_credentials(
    State(state): State<AppState>,
) -> AppResult<Json<Vec<models::CredentialSummary>>> {
    let db = state.db.lock().await;
    Ok(Json(db.list_credentials()?))
}

async fn create_credential(
    State(state): State<AppState>,
    Json(request): Json<CreateCredentialRequest>,
) -> AppResult<Json<models::CredentialSummary>> {
...
async fn list_strategies(
    State(state): State<AppState>,
) -> AppResult<Json<Vec<models::StrategySummary>>> {
    let db = state.db.lock().await;
    Ok(Json(db.list_strategies()?))
}

async fn create_strategy(
    State(state): State<AppState>,
    Json(request): Json<CreateStrategyRequest>,
) -> AppResult<Json<models::StrategySummary>> {
    let created = {
        let db = state.db.lock().await;
...
) -> AppResult<Json<StrategyDetailResponse>> {
    let db = state.db.lock().await;
    Ok(Json(db.strategy_detail(&strategy_id)?))
}
...
) -> AppResult<Json<Option<models::TradeRecord>>> {
    let symbol_override = query.symbol.map(|value| normalize_symbol(&value));
    let trade = run_strategy_once(&state, &strategy_id, symbol_override.as_deref()).await?;
    Ok(Json(trade))
}

async fn sync_strategy_broker(
    State(state): State<AppState>,
    Path(strategy_id): Path<String>,
) -> AppResult<Json<BrokerSyncState>> {
...
async fn collect_now(State(state): State<AppState>) -> AppResult<Json<CollectResponse>> {
...
async fn collect_once(state: &AppState) -> AppResult<CollectResponse> {
    let tracked_symbols = {
        let db = state.db.lock().await;
        let mut symbols = db.tracked_symbols_union(&state.config.default_watchlist)?;
        symbols.extend(db.watchlist_symbols_union()?);
...
async fn resolve_alpaca_credential(
    state: &AppState,
...
async fn sync_strategy_broker_state(
    state: &AppState,
    strategy_id: &str,
    ...
    let credential = resolve_alpaca_credential(state, strategy.credential_id.as_deref(), false)
        .await?
        .ok_or_else(|| AppError::Validation("missing Alpaca credential".to_string()))?;
    ...
    let db = state.db.lock().await;
    db.strategy_broker_sync(strategy_id)?
    ...
    let mut candidates = option_contracts
        ...
        .filter_map(|contract| {
            ...
            let delta_score = contract
                .delta
                .map(|delta| (delta.abs() - strategy.option_target_delta).abs())
...
fn option_structure_label(preset: OptionStructurePreset) -> &'static str {
    match preset {
...
fn fallback_delta_score(contract: &OptionContractSnapshot, underlying_price: f64, target_type: &str) -> f64 {
...
async fn spawn_agent_loop(state: AppState, strategy_id: String) {
...
async fn abort_agent_loop(state: &AppState, strategy_id: &str) {
    let mut tasks = state.agent_tasks.lock().await;
    if let Some(handle) = tasks.remove(strategy_id) {
        handle.abort();
    }
...
async fn list_watchlists(State(state): State<AppState>) -> AppResult<Json<Vec<models::Watchlist>>> {
    let watchlists = {
        let db = state.db.lock().await;
        db.list_watchlists()?
    };
...
async fn create_watchlist(
    State(state): State<AppState>,
    Json(payload): Json<models::CreateWatchlistRequest>,
) -> AppResult<Json<models::Watchlist>> {
...
async fn update_watchlist(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(payload): Json<models::UpdateWatchlistRequest>,
) -> AppResult<Json<models::Watchlist>> {
    {
        let db = state.db.lock().await;
...
mod tests {
    ...
    fn sample_strategy() -> StrategyRecord {
        StrategyRecord {
            ...
            equity: 25_000.0,
            tracked_symbols: vec!["AAPL".to_string()],
            total_trades: 0,
            wins: 0,
            losses: 0,
            last_signal: None,
            last_run_at: None,
            run_interval_ms: 30_000,
        }
    }

    fn sample_quote() -> models::Quote {
        models::Quote {
    ...
    fn sample_contract(symbol: &str, delta: f64, bid: f64, ask: f64, strike: f64) -> OptionContractSnapshot {
        OptionContractSnapshot {
            contract_symbol: symbol.to_string(),
            underlying_symbol: "AAPL".to_string(),
            provider: DataProvider::Alpaca,
            option_type: "call".to_string(),
            expiration: "2026-05-15T00:00:00+00:00".to_string(),
            strike,
            bid: Some(bid),
            ask: Some(ask),
            last: None,
    ...
    #[test]
    fn resolve_option_contract_prefers_target_delta_and_liquidity() {
        let strategy = sample_strategy();
        let quote = sample_quote();
        let contracts = vec![
            sample_contract("AAPL260515C00210000", 0.48, 4.8, 5.6, 210.0),
            sample_contract("AAPL260515C00205000", 0.31, 5.0, 5.3, 205.0),
            sample_contract("AAPL260515C00200000", 0.29, 5.0, 6.5, 200.0),
        ];

        let resolved = resolve_option_contract(&strategy, &quote, &contracts).expect("contract");
        assert_eq!(resolved.contract_symbol, "AAPL260515C00205000");
    }

    #[test]
    fn prepare_option_trade_uses_integer_contract_quantity() {
        let mut strategy = sample_strategy();
        strategy.cash_balance = 1_000.0;
        let quote = sample_quote();
        ...
        let contracts = vec![sample_contract("AAPL260515C00205000", 0.30, 2.0, 2.1, 205.0)];

        let prepared = prepare_option_trade(&strategy, None, "AAPL", &quote, &signal, &contracts, true)
            .expect("prepared");
        let TradePreparationOutcome::Ready(trade) = prepared else {
            panic!("expected ready trade");
        };

        assert_eq!(trade.local.quantity, 4.0);
        assert!(matches!(
            trade.broker_order,
            Some(AlpacaOrderRequest::Single {
                quantity,
                order_type: AlpacaOrderType::Limit { .. },
                ..
            }) if (quantity - 4.0).abs() < f64::EPSILON
        ));
    }

    #[test]
    fn prepare_bull_call_spread_builds_multileg_order() {
        let mut strategy = sample_strategy();
        strategy.option_structure_preset = OptionStructurePreset::BullCallSpread;
        strategy.cash_balance = 1_000.0;
        let quote = sample_quote();
        let signal = StrategySignal {
            action: SignalAction::Buy,
        ...
        let contracts = vec![
            sample_contract("AAPL260515C00200000", 0.30, 2.0, 2.1, 200.0),
            sample_contract("AAPL260515C00205000", 0.18, 0.8, 0.9, 205.0),
        ];

        let prepared =
            prepare_option_trade(&strategy, None, "AAPL", &quote, &signal, &contracts, true)
                .expect("prepared");
        let TradePreparationOutcome::Ready(trade) = prepared else {
            panic!("expected ready trade");
        };

        assert_eq!(trade.local.asset_type, "option_spread");
        assert_eq!(trade.local.legs.len(), 2);
        assert!(matches!(
            trade.broker_order,
            Some(AlpacaOrderRequest::MultiLeg { ref legs, .. }) if legs.len() == 2
        ));
    }
}
