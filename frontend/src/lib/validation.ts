...
export type StrategyErrors = Record<string, string>;

export function validateStrategyDraft(draft: StrategyDraft): StrategyErrors {
  ...
  if (draft.asset_class_target === "options") {
    ...
    if (isNaN(dteMin) || dteMin < 1) {
      errors.option_dte_min = "Min DTE must be at least 1";
    }

    const dteMax = Number(draft.option_dte_max);
    if (isNaN(dteMax) || dteMax < 1) {
      errors.option_dte_max = "Max DTE must be at least 1";
    }
